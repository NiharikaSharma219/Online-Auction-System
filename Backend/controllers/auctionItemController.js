...
export const addNewAuctionItem = catchAsyncErrors(async (req, res, next) => {
    ...
    if(!req.files || Object.keys(req.files).length === 0) {
        ...
        if(new Date(startTime) >= new Date(endTime)){
            return next(new ErrorHandler("Auction ending time must be less than ending time.", 400));
        }

        const alreadyOneAuctionActive = await Auction.findOne({ createdBy: req.user._id, endTime: { $gt: Date.now() } });
...
export const republishItem = catchAsyncErrors(async (req, res, next) => {
    ...
    if (!req.body || !req.body.startTime || !req.body.endTime) {
        return next(new ErrorHandler("Please provide new startTime and endTime to republish.", 400));
    }
    ...
    const createdBy = await User.findByIdAndUpdate(req.user._id,{ unpaidCommissions: 0},
        {
            new: true,
            runValidators: false,
            useFindAndModify: false,
