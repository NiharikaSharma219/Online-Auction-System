...
export const register = catchAsyncErrors(async(req, res ,next) => {
    ...
    const {userName, email, password, phone, address, role, bankAccountNumber, bankAccountName, bankName, ifscCode, upiId} = req.body;

    if(!userName || !email || !phone || !password || !address || !role){
        return next(new ErrorHandler("Please fill full form.", 400));
    }
    if(role === "Auctioneer"){
        if(!bankAccountNumber || !bankAccountName || !bankName || !ifscCode){
