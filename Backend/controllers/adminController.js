...
export const monthlyRevenue = catchAsyncErrors(async (req, res, next) => {
    ...
    const transformDataToMonthlyArray = (payments, totalMonths = 12) => {
        ...
        payments.forEach((payment) => {
          if (payment._id && payment._id.month) {  
    ...
    const biddersArray = new Array(12).fill(0);
    const auctioneersArray = new Array(12).fill(0);

    bidders.forEach((bidder) => {
        if (bidder.createdAt) {
            const month = new Date(bidder.createdAt).getMonth(); // 0-indexed (0 = Jan, 6 = July)
            biddersArray[month] += 1;
        }
    });

    auctioneers.forEach((auctioneer) => {
        if (auctioneer.createdAt) {
            const month = new Date(auctioneer.createdAt).getMonth();
            auctioneersArray[month] += 1;
        }
    });
